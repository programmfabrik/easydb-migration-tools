...
def init_error_log(start: datetime = None):
    ...
    """
    init_logfile(ERROR_LOG_FILE, start=datetime.now() if start is None else start)
...
def init_info_log(start: datetime = None):
    ...
    """
    init_logfile(INFO_LOG_FILE, start=datetime.now() if start is None else start)
...
def debugline():
    ...
    return '%s:%d' % (os.path.basename(frameinfo.filename), frameinfo.lineno)
...
def print_traceback(e: Exception):
    ...
    traceback_array = [repr(e), traceback.format_list(full_tb) + exc_line]

    traceback_str = '%s\n%s' % (traceback_array[0], ''.join(traceback_array[1]))
...
def sqlite3_execute(con: sqlite3.Connection, query: str, params=[], debug=False):
    ...
    if debug:
        log_info(
            'QUERY: %d rows (db: %d ms)' % (len(res), (t2 - t1).microseconds / 1000)
        )
...
def sqlite3_select(con: sqlite3.Connection, query: str, params=[], debug: bool = False):
    ...
    if debug:
        log_info(
            'SELECT: %d rows (db: %d ms, parse: %d ms)'
            % (len(res), (t2 - t1).microseconds / 1000, (t3 - t2).microseconds / 1000)
        )

    return result_rows


def sqlite3_count_rows(
    con: sqlite3.Connection,
    table: str,
    specific_select: str = None,
    debug: bool = False,
):
    """
...
def save_json_to_gzip_file(
    outputfolder: str, filename: str, data: dict, compression: int
) -> str:
    """
    ...
    filename += '.gz'
    with gzip.open(
        '%s/%s' % (outputfolder, filename), 'wb', compresslevel=compression
    ) as f:
...
class ObjectPayloadManager(object):
    ...
    def empty_payload(cls, import_type: str):
        ...
        return {'import_type': import_type, obj_key: []}, obj_key
    ...
    def version_invalid(cls, obj, objecttype, version, ref_col):
        ...
        if obj[objecttype]['_version'] != version:
            log_error(
                obj[objecttype][ref_col],
                '_version =',
                obj[objecttype]['_version'],
                '!= ',
                version,
            )
            ...
            if not '_version:auto_increment' in obj[objecttype]:
                log_error(
                    obj[objecttype][ref_col],
                    '_version = 0 but _version:auto_increment not set -> skip',
                )
                return True
            if not obj[objecttype]['_version:auto_increment']:
                log_error(
                    obj[objecttype][ref_col],
                    '_version = 0 but _version:auto_increment not TRUE -> skip',
                )
                return True
    ...
    def save_payloads(
        ...
        while has_more:
            ...
            filename = 'db__v%d__%s__batch_%04d__size_%04d.json' % (
                version,
                objecttype,
    ...
    def save_hierarchical_payloads(
        self,
        manifest: dict,
        outputfolder: str,
        objecttype: str,
        ...
        for level in objects_tree:
            ...
            while has_more:
                ...
                filename = 'db__v%d__%s__level_%02d__batch_%04d__size_%04d.json' % (
                    version,
                    objecttype,
                    int(level),
                    batch,
    ...
    @classmethod
    def group_tree_by_levels(
        cls,
        hierarchie_list: dict,
        parent: str = None,
        tree_levels: dict = {},
        level: int = 0,
    ):
        ...
        for leaf in hierarchie_list[parent]:
            ...
            if len(tree_levels[level_key]) % 1000 == 0:
                log_info(
                    'build hierarchical tree | level',
                    level,
                    '| handled',
                    len(tree_levels[level_key]),
                    'objects',
                )
            ...
            cls.group_tree_by_levels(
                hierarchie_list, parent=leaf, tree_levels=tree_levels, level=level + 1
            )
    ...
    def merge_object(cls, ref, new_obj: dict, old_obj: dict, path: list = []):
        ...
        if len(path) == 0:
            ...
            if objecttype != old_objecttype:
                raise Exception('could not merge objects: objecttypes do not match')
            ...
            old_obj[objecttype], updated = cls.merge_object(
                ref, new_obj[objecttype], old_obj[old_objecttype], path=[objecttype]
            )
        ...
        for obj_key in new_obj:
            ...
            if isinstance(new_v, dict) and isinstance(old_v, dict):
                obj[obj_key], _updated = cls.merge_object(
                    ref, new_v, old_v, path=path + [obj_key]
                )
                if _updated:
                    updated = True
                continue
    ...
    def merge_export_object(
        self, objecttype: str, ref_col: str, obj: dict, parent_key: str = None
    ):
        """
        ...
        if not objecttype in obj:
            log_error(
                'merge_export_object: invalid object: given objecttype %s not in object'
                % (objecttype)
            )
            return 0, False
        if not ref_col in obj[objecttype]:
            log_error(
                'merge_export_object: invalid object: given reference field %s not in object.%s'
                % (ref_col, objecttype)
            )
            return 0, False
        ...
        if ref is None:
            log_error(
                'merge_export_object: invalid object: given reference field %s in object.%s is None'
                % (ref_col, objecttype)
            )
            return 0, False
        ...
        if parent_key is not None:
            ...
            if not self.insert_object_ref_into_hierarchie(
        ...
        if ref not in self.export_objects[objecttype]:
            if self.verbose:
                log_debug(
                    'insert new object of objecttype %s with ref %s' % (objecttype, ref)
                )
        ...
        if self.verbose:
            log_debug(
                'update existing object of objecttype %s with ref %s'
                % (objecttype, ref)
            )
        old_obj = self.export_objects[objecttype][ref]
        new_obj, updated = self.merge_object(ref, obj, old_obj)
        self.export_objects[objecttype][ref] = new_obj

        return 0, updated

    def insert_object_ref_into_hierarchie(
        self, obj: dict, objecttype: str, ref: str, ref_col: str, parent_key: str
    ):
        if not objecttype in obj:
            return False
    ...
    def save_batch(
        ...
        if compression < 1:
            ...
            filename = save_json_to_gzip_file(outputfolder, filename, data, compression)
