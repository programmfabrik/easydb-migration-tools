...
def load_collection_objects(
    ...
    db = destination.get_db()
    db.open()
    rows = db.execute('SELECT COUNT(*) FROM "easydb.ez_collection__objects" WHERE object_goid is NULL')
    ...
    while(loop):
        loop = False
        db.open()
        sql = 'SELECT * FROM "easydb.ez_collection__objects" co JOIN  "easydb.ez_collection" c on (co.collection_id = c.__easydb_id) where co."uploaded" is null'
        rows = db.execute(sql)
