...
SELECT origin_id, source_table_name FROM origin WHERE
    ...
    if len(pks) > 0:
        cmd += ",\nPRIMARY KEY ("+'"'+'","'.join(map(__str_to_unicode, pks))+'")'
...
def __copy_data_to_source (
    ...
    for (table_name, tb) in list(tables.items()):
        ...
        while True:
            ...
            if use_limit is not None and use_limit > 0 and offset + chunk > use_limit:
...
def __sqlite_get_schema(conn,
    ...
    for row in sql.fetchall():
        ...
        schema["tables"][tn] = {
            "table_name": row[1],
            "table_name_select_escaped": '"'+__str_to_unicode(row[1])+'"',
            "columns": []
            }
